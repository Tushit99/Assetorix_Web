...
const ResidentialRent = () => {     
    ...
    const handleAvailablefor = (value) => { 
        console.log(value);
            ...
            <Flex display={"flex"} margin={"20px auto"} alignItems={"flex-start"} gap={4} > 
             {/* ===================================  Property Sorting ================================= */}
                <Box flex={2} padding={5} boxShadow={"rgba(100, 100, 111, 0.2) 0px 7px 29px 0px"} >
                    ...
                    <Box margin={"15px auto"}> 
                        <Heading textAlign={"left"} size={"sm"} > Available for </Heading>
                        <Box display={"flex"} justifyContent={"left"} alignItems={"baseline"} flexWrap={"wrap"} gap={3} margin={"10px auto"} >
                            <button className={availableFor.includes("Family") ? style.bhkbtn : style.selectedbtn} onClick={(e) => handleAvailablefor(e.target.value)} value={"Family"} > {availableFor.includes("Family") ? <BsCheckLg /> : <BiPlus />} Family </button>
                            <button className={availableFor.includes("Single Men") ? style.bhkbtn : style.selectedbtn} onClick={(e) => handleAvailablefor(e.target.value)} value={"Single Men"} > {availableFor.includes("Single Men") ? <BsCheckLg /> : <BiPlus />} Single Men </button>
                            <button className={availableFor.includes("Single Women") ? style.bhkbtn : style.selectedbtn} onClick={(e) => handleAvailablefor(e.target.value)} value={"Single Women"} > {availableFor.includes("Single Women") ? <BsCheckLg /> : <BiPlus />} Single Women </button>
                        </Box> 
                    </Box> 
                </Box>
                {/* =========================== product List ====================== */}
